...
以下のスクリプトを **一時ファイルに書き出してから `bash` で実行**し、出力を解釈して処理する。
zsh 環境ではインライン実行時にグロブ展開エラーが発生するため、必ずファイル経由で実行すること。

**手順:**
1. Bash ツールで `mktemp` を実行し、出力された一時ファイルパス（例: `/tmp/tmp.XXXXXX`）を控える
2. Write ツールでその **実パス** にスクリプトを書き出す
3. Bash ツールで `bash <実パス>; rm -f <実パス>` を実行する（`<実パス>` は Step 1 の出力値に置換）
