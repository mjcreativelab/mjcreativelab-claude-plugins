...
| オプション        | 説明                                                   | 例                                                  |
| ----------------- | ------------------------------------------------------ | --------------------------------------------------- |
| `-p <プロンプト>` | PR のタイトル・本文・ラベル等に関する追加指示           | `/smart-pr -p レビュアー向け補足にパフォーマンスの懸念を書いて` |

`-p` で渡されたプロンプトは、PR 本文の作成・更新時に考慮される。
例:

- `/smart-pr -p ドラフトで作って` → ドラフト PR として作成
- `/smart-pr -p レビュアー向け補足に移行手順を書いて` → 補足セクションに指定内容を追記
- `/smart-pr -p タイトルを変えて` → タイトルの変更を許可
