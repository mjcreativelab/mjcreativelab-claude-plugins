...
- 対象が PR と判別できた場合、確認ゲートを経て PR に Review（`event: "COMMENT"`）を投稿する。Judge（Phase 2）呼び出し不能時は投稿ゲートに到達しない
...
# コードレビュー
/mjc-code-develop-tools:code-reviewer [<PR番号|branch|ref..ref|path>]
