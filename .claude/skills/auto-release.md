...
例: `-p 1.2.0 にして` / `-p mjc-git-workflow だけ`

## バージョン体系

パッケージごとに独立したバージョンを管理する。

- **タグ形式**: `<package-name>@<semver>`（例: `mjc-git-workflow@1.1.0`）
- **バージョン格納先**: `.claude-plugin/marketplace.json` の各 plugin の `version` フィールド
...
`.claude-plugin/marketplace.json` を読み取り、登録されているパッケージ一覧を取得する。
...
- **タグが存在する** → そのバージョンを前回バージョンとして使用（例: `mjc-git-workflow@1.0.0` → `1.0.0`）
...
例: `release/mjc-git-workflow-v1.1.0`

既に作業ブランチにいる場合は、そのブランチ上で作業を続ける。

### 6. marketplace.json の更新

`.claude-plugin/marketplace.json` の対象パッケージの `version` フィールドを新バージョンに更新する。
...
git tag -f <package-name>@<version>
git push origin <package-name>@<version> --force
```
...
- `--no-verify` は使わない
- タグの force push はステップ 12 のタグ付け直しのみ許可
- marketplace.json 以外のファイルは変更しない（ソースコードの変更は事前にコミット済みであること）
